"""Measure the effect of IVF-Flat n_clusters on recall, latency, and candidates."""
...
def main() -> None:
    ...
    output_path = ROOT / "experiments" / "results" / "n_clusters_experiment.json"
    ...
    # Keep n_probe fixed so that only n_clusters changes.
    n_probe = 8
    cluster_values = [50, 100, 200]
    ...
    for n_clusters in cluster_values:
        print(f"Running n_clusters={n_clusters}, n_probe={n_probe}...")
        ...
        print(
            f"  Recall@1={result.recall.get(1, 0.0):.4f}, "
            f"Recall@5={result.recall.get(5, 0.0):.4f}, "
            f"Recall@10={result.recall.get(10, 0.0):.4f}"
        )
        print(
            f"  Build time={result.build_time:.4f} s, "
